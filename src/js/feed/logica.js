import { dropdownfeed, dropdownporhacer, dropdownsugerenciasamigos, dropdownsugerenciasthingos, dropdowntrending, perfilbanner, perfilbio, perfilfoto, perfilnick, perfilnumamigos, perfilnummentiras, perfilnumthingos, perfilnumverificaciones, perfilusername } from "./referencias"
...
export function thingoLoader(pagina) {
    fetch("http://localhost:3000/api/thingos" , {
    method : 'GET',
    headers : {
        'session_id' : sessionStorage.getItem("session_id")
    }
                ...
                <li class="py-3 sm:py-4">
                    <div class=""> <!-- ejemplo de entrada del feed -->
                        <div class="flex items-center space-x-2 rtl:space-x-reverse"> <!-- fecha del thingo -->
                            <span class="text-sm font-normal text-gray-500 dark:text-gray-400">${element}</span>
                        </div>
                        <div class="flex flex-col justify-between w-full leading-1.5 p-4 border-gray-200 bg-gray-100 rounded-e-xl rounded-es-xl dark:bg-gray-700">
                            ...
                            <div class="w-full dark:text-white flex justify-start">
                                <div class="min-w-max"> <!-- img del usuario que hizo el thingo -->
                                    <img class="w-20 h-20 rounded-full mr-6" src="${element}">
                                </div>
                                <div class="flex flex-col justify-between">
                                    <div><!-- nickname del usuario -->
                                        <span class="mb-2 text-xl font-extrabold">${element}</span>
                                        <!-- Mensaje -->
                                        <p class="mb-3 font-normal text-gray-700 dark:text-gray-400">${element}</p>
                                    </div>
                                    <span class="mb-2 text-xl font-extrabold text-red-600">Y tu ${element.hecho_por_usuario ? "no" : "tambien"}</span>
                                </div> 
                            </div>
                    ...
                    <div class="flex flex-row justify-center align-middle items-center w-full p-2 min-h-max dark:text-white">
                        ...
                        <div class="flex flex-row lg:flex-col-reverse w-full max-h-max p-2 ps-20 bg-gray-100 dark:bg-gray-700 border-gray-200 rounded-r-xl">
                            <div class="flex flex-col lg:flex-row justify-center lg:justify-end align-middle">
                                <a onclick="${}" class="inline-flex items-center justify-center w-10 h-10 sm:me-2 text-sm font-semibold text-gray-800 bg-gray-100 rounded-full dark:bg-gray-700 dark:text-gray-300 cursor-pointer">
                                    <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 12h14m-7 7V5"/>
                                    </svg> 
                                </a>
